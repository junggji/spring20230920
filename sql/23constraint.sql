...
CREATE TABLE my_table13 (
  col1 INT NULL , -- null 허용 (안써도 기본으로 허용)
...
VALUE (66); -- not ok (null 허용 X)

SELECT * FROM my_table13;


-- UNIQUE : 해당 컬럼에 유일한 값만 허용(중복되는 값 XXXX)
...
CREATE TABLE my_table17(
    col1 INT,
    col2 INT DEFAULT 100, -- 값을 넣지않으면 100
    col3 VARCHAR(10) DEFAULT 'empty', -- 값을 넣지않으면 'empty'
    col4 DATETIME DEFAULT NOW() -- 값을 넣지않으면 현재 일시
);
...
CREATE TABLE my_table18(
    col1 INT,
    col2 INT DEFAULT 300, -- 굳이 NULL넣으라면 가능
