...
//                System.out.println("name = " + name);
    ...
    //직원 table
    @GetMapping("sub4")
    public void method4(@RequestParam(value = "t", defaultValue = "lname") String searchOption,
                        @RequestParam(value = "k", defaultValue = "") String keyword,
                        Model model) throws SQLException {
        ...
        if (searchOption.equals("lname")) {
            sql += "FirstName LIKE ?";
        } else if (searchOption.equals("fname")) {
            sql += "LastName LIKE ?";
        } else if (searchOption.equals("note")) {
            sql += "Notes LIKE ?";
        }
